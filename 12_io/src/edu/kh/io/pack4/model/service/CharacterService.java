...
public class CharacterService {
	...
	/* 문자 기반 스트림
	 * - 2byte 문자 단위로 입/출력하는 스트림
	 * 
	 * - Reader (입력) / Writer(출력) 최상위 크래스
	 * 
	...
	public void fileInput() {
		...
		try {
			...
			while(true) {
				...
				if (line == null ) break; // 더이상 읽어올 내용이 없다면 반복 종료
	...
	public void fileOutput() {
		...
		try {
			...
			bw.write(sb.toString()); //StringBuilder타입인 sb를 String으로 바꾸어 sb에 저장된 내용을 연결된 파일에 출력(쓰기)
			bw.flush();
