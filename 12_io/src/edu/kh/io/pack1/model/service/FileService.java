...
public class FileService {
	...
	public void method1() {
		...
		// 제일 앞에 "/"로 시작한다는 의미 = 현재 실행중인 프로젝트의 루트 디렉토리
		// (루트 디렉토리 == 최상위 폴더 : root, 절대 경로의 기준점.)
		// 즉, 드라이브를 의미함.
		
		// 절대경로 작성법 : 
		// 만약 명시적으로 맨앞에 C: , D:라고 작성 시 해당 드라이브에 폴더가 생성된다.
		// C:
		//  ㄴ io_test
		//			ㄴ 20251014
		
		// 상대경로 작성법
		// 맨 앞에 '/' 없이 io_test/20251014 로 작성 시 해당 프로젝트가 '현재 위치'에 생성된다.
		// 현재 위치 == '해당 프로젝트' 안에 io_test/20251014 이 생성된다.
		// 현재 프로젝트 : 12_io
		//					ㄴ io_test
		//							ㄴ 20251014
		...
		// 폴더가 존재하지 않으면 생성
		if ( !directory.exists() ) { //파일/디렉토리가 존재하면 true, 아님 false
			// 존재하지 않으면 폴더를 모두 생성 (모두 생성일 경우 mkdir's'를 사용)
			directory.mkdirs();
			
			System.out.println(directory.getName()); // 파일 이름을 반환
			System.out.println(directory.getPath()); // 전체 경로를 반환
		}
	...
	public void method2() {
		...
		// 문자열에서 "\" 는 이스케이프 문자로 사용된다.
		// -> 역슬래시 자체를 문자로 사용하고 싶을 땐 2개를 사용해야 한다.
		// -> \\
		File file = new File("\\io_test\\20251014\\파일생성.txt");
		
		if (!file.exists() ) { // 이 경로에 해당 파일이 존재하지 않는다면 파일 생성
			// 파일 생성 코드
	...
	public void method3() {
		...
		for(File f : files) {
			...
			// 마지막으로 수정한 날짜
			long lastModified = f.lastModified(); // 예) 17351253124 로 나옴.
			
			// java.text.SimpleDateFormat 
			//  ㄴ 간단히 날짜 형식을 지정할 수 있는 객체
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd a h:mm");
														//예) 2025-10-14 오후 12:30 으로 나오게 바꿈.
			// String에서 반환하는 SimpleDateFormat.format(long) 메서드 :
			// 매개변수 long 값을 지정된 패턴 형식(예시 : "yyyy-MM-dd a h:mm")으로 변환하여
			//문자열 형태로 반환하는 메서드.
			...
			if(f.isDirectory()) size = ""; // f가 폴더라면 사이즈를 빈문자열로 변경
			// -> 폴더 크기를 구하는 메서드는 따로 존재하지 않음.
			// -> 구하려면 폴더 안의 모든 파일을 순회하여 각 파일 크기 합산하여 사용
