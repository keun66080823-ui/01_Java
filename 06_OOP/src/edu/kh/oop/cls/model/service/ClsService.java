...
public class ClsService /*extends Student*/ {
	...
	// 클래스 접근제한자 확인하기
		...
		// (defalut) 클래스는 같은 패키지 내에서만 사용 가능
		// 다른 패키지라 import도 불가능
		
		// 필드 접근제한자 확인하기
		// 상속 관계에서 직접 접근 가능 범위 테스트
		// * 상속받으면 부모의 것도 마치 내것처럼 사용 가능
		
//		System.out.println(v1); // public 가능
//		System.out.println(v2); // protected 가능
//		System.out.println(v3); // (default) 다른 패키지라 사용 불가
//		System.out.println(v4); // 해당 클래스 내부가 아닌 다른 클래스라 사용 불가
	...
	public void ex2() {
		...
		// *** static이 붙은 변수(필드)는
		//	  '클래스명.변수명'으로 사용함 ***
		System.out.println(Student.schoolName); // std라는 객체를 생성하지 않고도 접근 가능.
	...
	public void ex3() {
		...
		User u1 = new User(); //User()는 기본 생성자.
		// 기본 생성자를 개발자가 직접 만들지 않았다면
		// 컴파일러가 자동으로 기본 생성자 만들어줌.
		...
		// 이유 : 같은 기본생성자로 User 객체를 생성하고 있기 때문에
	...
	public void ex4() {
		...
		User u2 = new User("user02", "pass02"); // 매개변수 2개짜리 생성자로 객체 생성
		User u3 = new User("user03", "pass03", "김킬동", 20, '남');
		// 매개변수 개 생성자(전부 초기화)
		
		System.out.printf("u1 : %s / %s / %s / %d / %c\n",u1.getUserId(), u1.getUserId(), u1.getUserName(), u1.getUserAge(), u1.getUserGender());
		System.out.printf("u2 : %s / %s / %s / %d / %c\n",u2.getUserId(), 
				u2.getUserId(), u2.getUserName(), 
				u2.getUserAge(), u2.getUserGender());
		System.out.printf("u3 : %s / %s / %s / %d / %c\n",u3.getUserId(), 
				u3.getUserId(), u3.getUserName(), 
