...
public class ServerService {
	...
	public void serverStart() {
		...
		// 단, 1023번 이하는 이미 사용중인 경우가 많으니 피하는 게 좋은. 컴퓨터 내 기본적으로 존재하는 프로그램들이 사용함.
		...
		try {
			...
			// 2025년 10월 16일 10:39:30 [서버 접속 성공] 메세지를 클라이언트에게 보내기
			Date now = new Date(); // java.util.Date : 자바에서 시간을 나타내는 객체
			// new Date() 객체 생성 시(이 코드가 수행될 때) 현재 시간이 저장되어있다.
			
			// SimpleDateFormat : 날짜 형식을 간단히 지정하는 객체
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy년 MM월 dd일 HH:mm:ss"); //대문자 HH라면 24시간 표기법이다.
			...
			pw.flush(); // flush() : 스트림 내용을 밀어냄 / pw도 출력용 스트림이므로 flush가 필요.
			
			// -------------------------------------------------------
			
			// 클라이언트 -> 서버 메세지 전달 받기(읽기 : 입력)
