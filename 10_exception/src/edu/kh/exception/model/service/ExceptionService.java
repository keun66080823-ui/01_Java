...
public class ExceptionService {
	...
	public void ex1( ) {
		...
		try {
			...
			System.out.println("0으로 나눌 수 없음.");
			e.printStackTrace();
			// 발생한 예외에 대한 메서드와 그 위치에 대한 모든 내용을 출력
			// 예외 발생 지점 추적 메서드
			
		}
		
		// 발생하는 예외 중 일부 예외는 try-catch 사용 안 해도
		// if-else 예외 상황 방지 가능
	...
	public void ex3() {
		// try - catch - finally
		// finally : try 구문에서 예외 발생 유무 관계없이 무조건 마지막에 수행
		...
		try {
			...
			e.printStackTrace(); // 상세히 예외 추적해주는 메서드 (어느 위치의 어느 코드에서 예외가 발생했는지 추적)
		...
		// try-with-resoirces : finally에서 일일이 close() 호출하지 않아도
		// 자동으로 리소스를 닫아주는 방식
	...
	public void ex5() throws UserException{
		...
		// 자바가 미리 만들어둔 예외 클래스 중 마땅한 게 없을 때
