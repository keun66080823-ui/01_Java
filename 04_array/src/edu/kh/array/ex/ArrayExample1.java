...
public class ArrayExample1 {
	...
	public void ex1() {
		...
		arr = new int[3];
		// new : "new 연산자" 라고 하며
		//		Heap 메모리 영역에 새로운 공간(배열, 객체)을 할당하겠다는 의미로 사용.
		...
		arr[1] = 50;
		arr[2] = 100;
//		arr[3] = 500; // 런타임 에러 메세지 : Index 3 out of bounds for length 3 
											   //배열 범위를 벗어나 에러 발생
		...
		// Arrays.toString(배열명); -> String 형으로 반환.
...
//		arr[4] = 500;
		...
		//해석 : 배열의 인덱스 범위를 벗어남:
		//		길이를 4칸짜리로 만든 배열인데, 없는 인덱스 번호인 4에 접근했다		
	...
	public void ex8() {
		...
		// 입력한 문자열을 한글자씩 잘라내어 char 배열의 모든 요소에 순서대로 값 대입
		for (int i = 0; i<arr.length; i++) { 
		...
		// charAt(index) 메서드를 이용하여 index 번째 존재하는 문자 하나를
		// char형으로 받도록 해야 한다.
