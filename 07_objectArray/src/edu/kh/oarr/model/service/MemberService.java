...
public class MemberService {
	...
	public MemberService(){
		...
		memberArr[1] = new Member("user02", "pass02", "계보린", 
				25, "경기");
		memberArr[2] = new Member("user03", "pass02", "고길동", 
				45, "강원");
...
//				}
	...
	public String signUp() {
		...
		// -> 빈자리가 있을 때 : 빈 공간에 인덱스 번호 얻어오기
		// -> 없을 때 : -1 반환
		int index = emptyIndex();
		
		if(index==-1) { //비어 있는 공간 없음 -> 회원가입 불가!
		...
		for(int i = 0; i < memberArr.length; i++) {
			...
			if(memberArr[i] != null) { // 회원 정보가 있을 경우
				...
				&& memberArr[i].getMemberPw().equals(memberPw) ) {
					...
					// loginMember에 현재 접근 중인 memberArr[i]
					// 저장된 주소를 얕은복사
					loginMember = memberArr[i];
					break; // 더 이상 같은 아이디, 비밀번호가 
...
//					memberArr[i].setMemberName(newName);
//					memberArr[i].setMemberAge(newAge);
//					memberArr[i].setRegion(newRegion);					
//				}
//			}
			return 1;
		}else {
